...
public class LargeSteamCompressor {
    public static final MachineDefinition LARGE_STEAM_COMPRESSOR = PhoenixRegistration.REGISTRATE.multiblock("large_steam_compressor", SteamParallelMultiblockMultiRecipeTypeMachine::new)
            ...
            .pattern(multiblockMachineDefinition -> FactoryBlockPattern.start().aisle("BCCCB", "BBCBB", "BBCBB", "BBBBB", "BBBBB")
                    .aisle("CDDDC", "BDBDB", "BDEDB", "BBDBB", "BBBBB")
                    .aisle("CDDDC", "CBBBC", "CEFEC", "BDDDB", "BBGBB")
                    .aisle("CDDDC", "BDBDB", "BDEDB", "BBDBB", "BBBBB")
                    .aisle("BCCCB", "BBHBB", "BBCBB", "BBBBB", "BBBBB")
                    .where("B", Predicates.any())
                    .where("C", blocks(ForgeRegistries.BLOCKS.getValue(ResourceLocation.parse("gtceu:steam_machine_casing"))).setMinGlobalLimited(6)
                            .or(Predicates.abilities(PartAbility.STEAM_IMPORT_ITEMS).setPreviewCount(1))
                            .or(Predicates.abilities(PartAbility.STEAM_EXPORT_ITEMS).setPreviewCount(1))
                            .or(Predicates.abilities(PartAbility.STEAM).setPreviewCount(1)))
                    .where("D", blocks(ForgeRegistries.BLOCKS.getValue(ResourceLocation.parse("gtceu:industrial_steam_casing"))))
                    .where("E", Predicates.blocks(CASING_BRONZE_BRICKS.get()))
                    .where("F", Predicates.blocks(FIREBOX_BRONZE.get()))
                    .where("G", Predicates.blocks(ForgeRegistries.BLOCKS.getValue(ResourceLocation.parse("gtceu:bronze_machine_casing"))))
